...
public class InsnGen {
	...
	public String assignVar(InsnNode insn) {
		try {
			...
			Log.e(tag, "Assign var codegen error", e);
		}
		return "<error>";
	}
		...
		if (callMthNode.getMethodInfo().getArgumentsTypes().isEmpty()) {
			...
			for (RegisterArg r : inlArgs) {
				if (r.getRegNum() >= regs.length) {
					Log.w(tag, "Unknown register number {} in method call: {}, {}" + r + ", " + callMthNode + ", " + mth);
				} else {
					InsnArg repl = regs[r.getRegNum()];
					if (repl == null) {
						Log.w(tag, "Not passed register {} in method call: {}, {}" + r + ", " + callMthNode + ", " + mth);
					} else {
